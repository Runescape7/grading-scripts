...
cd Lab4 # folder for local repository

# add and fetch remote for the student repository, if it fails, rollback and exit
git remote add -f ${1%%-*} git@bitbucket.org:${1%%-*}/lab4.git || \
...
~/Dropbox/a2pdf-1.13-OSX-Intel/a2pdf --noperl-syntax Lab4Code/ShapeDrawer.java > ../grading/Lab4/$1.pdf # convert the Java file to PDF
